...
**Plans**: 7 plans
  ...
  - [x] Gap closure (post-verificación, 2026-06-19) — SC#3: los 8 textos editoriales de NIVEL sección/grupo que el diff per-card no veía (eyebrows+intros de gastronomía/arte/arquitectura + gastro-intro de quinto quarto/ghetto) capturados verbatim en `TripSchema.sections` y `food.groupIntro`; `migration-diff` extendido (`extractSectionMeta`/`extractGroupIntro` + test de nivel sección con fixtures negativos) para que no regrese. `test:data` 295 verde. Commits b7a4a93, fbe8f9c
...
**Plans**: 5 plans
  ...
  - [x] 03-05-PLAN.md — Wire app.vue/pages/routing + parity head (D-09); Playwright shell+routing+FOUC+icon parity + human golden sign-off [Wave 4]
**UI hint**: yes
...
**Plans**: 4 plans
  - [x] 07-01-PLAN.md — Coliseo extra-marker (TripSchema + trip.yml) + 3 pure utils (mapMarkers/mapOffline/svgMotifs) + their Vitest specs [Wave 1]
  - [x] 07-02-PLAN.md — LeafletMap.client.vue island (dynamic import in onMounted, divIcon markers, popups, fitBounds, offline heuristic) + TripView #mapa wiring (ClientOnly + same-size fallback) [Wave 2]
  - [x] 07-03-PLAN.md — Image fallback hero/detail (@error → motif SVG, provide/inject) + per-card notes persistence (roma-note-<slug>, onMounted) in MonumentCard + DetailPhoto.global [Wave 2]
  - [x] 07-04-PLAN.md — Self-contained Playwright parity spec (map/popups/offline/fallback/notes, SC#1–SC#7) + human paridad sign-off [Wave 3]
**UI hint**: yes
...
**Plans**: 7 plans
  ...
  - [x] 08-02-PLAN.md — Nuxt-vs-golden visual-diff spec (D-01/SC#1): 5th self-contained clone, VIEWS+settle()+A5 verbatim, 14 views vs the 56 frozen PNGs, port 5780 [Wave 1]
  - [x] 08-03-PLAN.md — Gate config + package.json scripts (D-01/D-03/D-04#1): pin snapshotPathTemplate to the frozen dir, testIgnore golden.spec.ts, pnpm verify [Wave 2]
  ...
  - [ ] 08-05-PLAN.md — tests/README.md (D-04/D-01): document both gate exclusions, the frozen-baseline invariant, and the recorded gate test count [Wave 4]
  - [ ] 08-06-PLAN.md — Run gate once + classify diffs (D-02) + green pnpm verify (SC#4): real -> fix component to 0.01, artifact -> justified threshold; baseline untouched [Wave 5]
...
| 8. Verificación de paridad | 4/7 | In Progress|  |
