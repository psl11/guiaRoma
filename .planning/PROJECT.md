...
---
*Last updated: 2026-06-22 — Fase 6 completada (derivados de datos: **búsqueda en cliente** con MiniSearch sobre un haystack superset de `card.textContent` —`useSearch` singleton + `SearchBox`, dropdown ≥2 chars/máx 8/"Sin resultados", resultado→`navigateToCard`— y **"ruta del día"** derivada de `day.cards` en orden —utils puras `pointFor`/`capStops`/`buildDirUrl`, URL Google Maps a pie, todas las paradas sin filtro de tipo (Saturday=8 incl. Vaticano+Auditorium), botón con ≥2 paradas y etiqueta condicional. Code review encontró y se corrigió CR-01 (el `stopPropagation` en captura de F5 se tragaba el clic del resultado: arreglado con el mecanismo `data-card` del original). Paridad verificada con Vitest puro + Playwright autocontenido 10/10 + sign-off humano. Pendiente: mapa Leaflet + fallback de imagen + notas (F7), pixel-diff total (F8))*
