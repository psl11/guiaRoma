...
---
*Last updated: 2026-06-23 — Fase 7 completada (isla client-only: **mapa Leaflet** como única isla `.client.vue` —import dinámico en `onMounted`, `<ClientOnly>` + `#fallback` del mismo tamaño, 39 marcadores `divIcon` por tipo, popups `<a href="#slug">` sin handler que navegan vía el listener en captura de F5 (guiados texto-solo), banner offline con la heurística exacta `tilesErrored>3 && tilesLoaded===0`, `nuxt generate` sin `window is not defined`—; **imagen-con-fallback** `@error`→SVG por `motif` (hero sustituye el contenedor; detail solo la `<img>`, conservando caption); **notas por ficha** en `localStorage['roma-note-<slug>']` leídas en `onMounted`. Lógica pura en `app/utils/{mapMarkers,mapOffline,svgMotifs}` + Vitest; paridad verificada con spec Playwright autocontenido 12/12 (SC#1–SC#7) + sign-off humano. Code review: 3 warnings resueltos en fase (limpieza del ciclo de vida del mapa + `slug` por prop, restaurando el valor multi-viaje). Pendiente: verificación de paridad **pixel total** (F8))*
